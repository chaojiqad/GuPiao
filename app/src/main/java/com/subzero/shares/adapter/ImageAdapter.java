...
import com.subzero.shares.utils.BitmapUtils;
...
public class ImageAdapter extends BaseAdapter implements AbsListView.OnScrollListener {
    ...
    public ImageAdapter(Context context, ArrayList<String> imageUrls,GridView photoWall) {
        ct=context;
...
//        loader = new SDCardImageLoader(ScreenUtils.getScreenW(), ScreenUtils.getScreenH());
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = LayoutInflater.from(ct).inflate(R.layout.photo_wall_item, null);
...
//        loader.loadImage(3,imageUrls.get(position) , holder.imageView);
        // 给ImageView设置一个Tag，保证异步加载图片时不会乱序
        holder.imageView.setTag(imageUrls.get(position));
        setImageView(imageUrls.get(position), holder.imageView);
...
//            // TODO Auto-generated method stub
    ...
    private Bitmap downloadBitmap(String imageUrl) {
        Bitmap bm = BitmapUtils.getBitmap(imageUrl);
